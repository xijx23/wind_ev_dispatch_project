...
`src/models/` 和 `src/analysis/` 暂不实现，留给对应组员继续完成。
...
| `src/models/` | 预留给 EV 聚合、EMS 调度、分解、价格响应等模型 |
| `src/analysis/` | 预留给结果检查、指标、图表和表格生成 |
| `docs/` | 变量字典、预处理接口和任务范围说明 |
| `results/` | 预留模型结果、图表和表格输出目录 |
| `report/` | 预留最终报告材料 |
